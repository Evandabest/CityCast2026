...
// Try to load cached Instagram data (generated by scripts/refresh-instagram.sh)
let cachedPosts: InstagramPost[] | null = null;
let cacheUpdated: string | null = null;
try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const cache = require("@/data/instagram-cache.json") as { posts: InstagramPost[]; updated: string };
  cachedPosts = cache.posts;
  cacheUpdated = cache.updated;
} catch {
  // No cache file yet — will fall back to live API or placeholders
}

export function getInstagramCacheDate(): string | null {
  return cacheUpdated;
}

export async function getInstagramPosts(limit = 12): Promise<InstagramPost[]> {
  // Use cached data first (locally downloaded images that never expire)
...
function getPlaceholderPosts(): InstagramPost[] {
  return placeholderImages.map((url, i) => ({
    id: `placeholder-${i + 1}`,
    caption: `CityCast moment #${i + 1} - Catch all the action from CCNY! Follow us for more updates. #CityCast #CCNY #NYC`,
    media_type: "IMAGE" as const,
    media_url: url,
    permalink: "https://instagram.com/citycast",
...
function getPlaceholderProfile(): InstagramProfile {
  return {
    ...
    username: "citycast",
    name: "CityCast",
    biography: "CCNY's student-run campaign media platform for the Undergraduate Student Government.",
